    ...
    public class CupomRepositorio : ICupomRepositorio
    {
        ...
        public CupomRepositorio()
        {
            using (IDataReader reader = DatabaseFactory.CreateDatabase("DefaultConnection").ExecuteReader(CommandType.Text,
                @"select c.idempresa, NOME_FANTASIA, c.IDCUPOM, c.DESCRICAO, c.CODIGO, c.STATUS, case when c.TIPO = 'C' then 'Cliente' when c.TIPO = 'G' then 'GERAL' else 'ND' end as tipo_cupom,
                    ...
                    join APP_EMPRESA e on e.IDEMPRESA = c.IDEMPRESA"))
            {
                while (reader.Read())
                {
                    Add(new Cupons
                    {
                        ...
                        IdCupom = Convert.ToInt32(reader["idCupom"].ToString()),
                        NomeCupom = reader["descricao"].ToString(),
                        NomeEmpresa = reader["nome_fantasia"].ToString(),
                        Codigo = reader["codigo"].ToString(),
                        Tipo = reader["tipo_cupom"].ToString(),
                        Inicio = Convert.ToDateTime(reader["data_inicio"].ToString()),
                        Final = Convert.ToDateTime(reader["data_fim"].ToString()),
                        Beneficio = reader["beneficio"].ToString(),
                        Valor = Convert.ToDouble(reader["valor"].ToString()),
                        Qtde_Geral = Convert.ToInt16(reader["qtde_geral"].ToString()),
                        Qtde_Cliente = Convert.ToInt16(reader["qtde_cliente"].ToString()),
